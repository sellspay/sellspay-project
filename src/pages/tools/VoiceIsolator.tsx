...
import { Upload, Download, Play, Pause, Mic, Music, Loader2, X, Volume2 } from "lucide-react";
...
export default function VoiceIsolator() {
  ...
  // Determine which track to show based on mode
  const primaryTrack = mode === "remove-vocals" 
    ? result?.no_vocals || result?.instrumental 
    : result?.vocals;
  const secondaryTrack = mode === "remove-vocals" 
    ? result?.vocals 
    : result?.no_vocals || result?.instrumental;
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">
                  This may take 1-3 minutes depending on the file length
                </p>
              </div>
            </div>
                ...
                <CardContent className="p-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">
                          {mode === "remove-vocals" ? "Instrumental" : "Vocals"}
                        </p>
                        <p className="text-sm text-muted-foreground">
                          {mode === "remove-vocals" ? "Karaoke version" : "Acapella version"}
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      ...
                      size="sm"
                      className="flex-1"
                      onClick={() => togglePlay("primary", primaryTrack.url)}
                    >
                      {playingTrack === "primary" ? (
                    ...
                    <Button
                      ...
                      size="sm"
                      className="flex-1"
                      onClick={() => downloadTrack(primaryTrack.url, primaryTrack.filename)}
                    >
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">
                          {mode === "remove-vocals" ? "Vocals" : "Instrumental"}
                        </p>
                        <p className="text-sm text-muted-foreground">
                          {mode === "remove-vocals" ? "Extracted vocals" : "Background music"}
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      ...
                      size="sm"
                      className="flex-1"
                      onClick={() => togglePlay("secondary", secondaryTrack.url)}
                    >
                      {playingTrack === "secondary" ? (
                        <><Pause className="w-4 h-4 mr-2" /> Pause</>
                      ) : (
                        <><Play className="w-4 h-4 mr-2" /> Play</>
                      )}
                    ...
                    <Button
                      ...
                      size="sm"
                      className="flex-1"
                      onClick={() => downloadTrack(secondaryTrack.url, secondaryTrack.filename)}
                    >
                      <Download className="w-4 h-4 mr-2" /> Download
                    </Button>
                  </div>
